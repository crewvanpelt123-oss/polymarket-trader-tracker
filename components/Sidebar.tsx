  ...
  return (
    <aside className="w-80 flex-shrink-0 bg-slate-950 border-r border-slate-800 flex flex-col h-full z-10">
      <div className="p-6 flex-1 flex flex-col min-h-0">
        ...
        <div className="flex flex-wrap p-1 bg-slate-900 rounded-xl border border-slate-800 mb-8 gap-0.5">
              ...
              <nav className="flex flex-col gap-1">
                ...
                {traders.map((t) => (
                  <div key={t.address} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm cursor-pointer border ${selectedTrader === t.address ? 'bg-indigo-600/10 text-indigo-400 border-indigo-500/20' : 'text-slate-400 border-transparent hover:bg-slate-900'}`} onClick={() => onSelect(t.address)}>
                    ...
                    <a href={`https://polymarket.com/profile/${t.address}`} target="_blank" rel="noreferrer" onClick={(e) => { e.stopPropagation(); }} className="flex-1 min-w-0 truncate font-medium hover:text-indigo-300 underline decoration-slate-600 hover:decoration-indigo-400 transition-colors">{t.username || truncAddr(t.address)}</a>
